...
export default function ProductsInfinitePage() {
  ...
  return (
    <section>
      <h1>Список автомобілів — приклад infinite scroll</h1>
      {canManage && (
        <div className="centerBtn">
          <button className="defaultButton">
            <Link to={ROUTES.PRODUCT_CREATE}>+ Додати автомобіль</Link>
          </button>
        </div>
      )}
      <ProductFilters brands={brands} owners={owners} filters={filters} onChange={setFilters} onApply={handleApplyFilters} onReset={handleResetFilters} />
      {error && <p>{error}</p>}
          ...
          <table>
            <tbody className="dataTable">
              {products.map((product, index) => {
                ...
                return (
                  <tr ref={isLast ? lastElementRef : null} key={product._id}>
      ...
      {!hasNextPage && products.length > 0 && <p style={{ textAlign: "center" }}>Більше автомобілів немає</p>}
    </section>
  );
}